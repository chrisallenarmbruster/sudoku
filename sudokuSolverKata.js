//including test data for those wanting to tinker with this code,
//but these test arrays are not used in the Kata.
//Scroll down for solver function.
...
//Main solver function - this is the Kata challenge.
//It's a backtracking approach
function sudoku(puzzle) {
  let blankCells = saveBlankCells(puzzle);
  console.log(solve(puzzle, blankCells)); //solve the puzzle and log answer to console
    ...
    while (i < blankCells.length) {
      ...
      while (!found && value <= limit) {
        if (testValue(puzzle, column, row, value)) {
          found = true; //value is legit so far, so write it to the puzzle and move to next blank cell
          puzzle[row][column] = value;
          i++;
        } else {
          value++; //value didn't work, let's try another number.
        }
      }
      if (!found) {
        //Time to backtrack
  ...
  //This function tests a possible value for a given position and
  //returns false if already used in its row, column or square.
...
return sudoku(testPuzzle2); //left this in here for those wishing to tinker with the code, but not used in the Kata.
